...
const API = process.env.NEXT_PUBLIC_PORTFOLIO_PREVIEW === 'true' ? '' : (process.env.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '');
let csrf = '';
async function request(path: string, init: RequestInit = {}) {
  if (!API) {
    throw new Error(
      'This public preview is read-only. Public live access is planned for a future update.'
    );
  }
...
function readable(s:string){const acronyms:Record<string,string>={pr:'PR',api:'API',csrf:'CSRF',e2b:'E2B',github:'GitHub',ai:'AI'};return s.toLowerCase().split('_').map(word=>acronyms[word]||word.charAt(0).toUpperCase()+word.slice(1)).join(' ')}
...
export default function Workspace(){
 ...
 useEffect(()=>{try{setCollapsed(localStorage.getItem('repopilot:sidebar')==='collapsed')}catch{}},[]);
 function toggleSidebar(){requestAnimationFrame(()=>document.getElementById('desktop-nav-toggle')?.focus());setCollapsed(value=>{const next=!value;try{localStorage.setItem('repopilot:sidebar',next?'collapsed':'expanded')}catch{}return next})}
 ...
 useEffect(()=>{if(!mobile)return;const key=(e:KeyboardEvent)=>{if(e.key==='Escape'){setMobile(false);document.getElementById('mobile-nav-toggle')?.focus()}};document.addEventListener('keydown',key);return()=>document.removeEventListener('keydown',key)},[mobile]);
 ...
 useEffect(()=>{
  ...
  request('/auth/session/').then(async d=>{
   if(epoch!==sessionEpoch.current)return;
   csrf=d.csrf_token;setUser(d.username||'');setCanOperate(d.can_operate===true);
 ...
 async function action(path:string, body:object={}){if(!canOperate){setError('Live actions are restricted to the workspace operator.');return;}setError('');setBusy(true);try{const data=await request(path,{method:'POST',body:JSON.stringify(body)});if(data.id)setTask(data);await refresh();return data}catch(e){setError((e as Error).message)}finally{setBusy(false)}}
 async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();if(dialog!=='login'&&!canOperate){setError('Live actions are restricted to the workspace operator.');return;}const f=new FormData(e.currentTarget);setError('');setBusy(true);try{
   if(dialog==='login'){
    ++sessionEpoch.current;setLoading(true);setTask(demo);setUser('');setCanOperate(false);setTasks([]);setRepos([]);
    try{
     const d=await request('/auth/login/',{method:'POST',body:JSON.stringify({username:f.get('username'),password:f.get('password')})});
     csrf=d.csrf_token;setUser(d.username);setCanOperate(d.can_operate===true);await refresh(d.username,true);
 ...
 return <div className={'app '+(collapsed?'sidebar-collapsed':'')}><a className="skip-link" href="#workspace-main">Skip to workspace</a>
  {mobile&&<button className="nav-scrim" aria-label="Close navigation" onClick={()=>setMobile(false)}/>}<aside id="workspace-navigation" aria-label="Workspace navigation" className={'sidebar '+(mobile?'open':'')}><button className="desktop-sidebar-close icon-button" aria-label="Collapse sidebar" title="Collapse sidebar" onClick={toggleSidebar}><PanelRightOpen size={20}/></button><button className="sidebar-dismiss icon-button" aria-label="Close navigation" onClick={()=>setMobile(false)}><X size={20}/></button>
   <a className="brand" href="/" aria-label="RepoPilot home"><img className="workspace-logo workspace-logo-light" src="/brand/repopilot-logo.svg" alt="" width="1779" height="486"/><img className="workspace-logo workspace-logo-dark" src="/brand/repopilot-logo-dark.svg" alt="" width="1779" height="486"/><span className="version">BETA</span></a>
   <button className="workspace-select" onClick={()=>setSection('Home')}><span className="avatar">N</span><span>{user || (API?'Operator workspace':'Portfolio preview')}<small>RepoPilot workspace</small></span><ChevronDown size={15}/></button>
   <button className="primary new-task" onClick={()=>setDialog(API?(user?(canOperate?'task':'connection'):'login'):'connection')}><Plus size={17}/>{API?'New task':'Live tasks · planned'}<span>＋</span></button>
   ...
   <div className="sidebar-repos"><p className="nav-caption">REPOSITORIES<button aria-label="Add repository" onClick={()=>setDialog(API?(user?(canOperate?'repository':'connection'):'login'):'connection')}><Plus size={14}/></button></p>{(repos.length?repos:API?[]:[demo.repository]).slice(0,5).map(r=><button className="repo-nav" key={r.id} onClick={()=>setSection('Repositories')}><Github size={16}/><span>{r.full_name.split('/')[1]}</span></button>)}</div>
   <a href="/guide/" className="navitem guide-nav"><FileCode2 size={18}/>How to use RepoPilot</a><div className="sidebar-bottom"><div className="boundary"><ShieldCheck size={18}/><div>You're in control<small>Review changes before publishing.</small></div></div><button className={'navitem '+(section==='Settings'?'selected':'')} onClick={()=>setSection('Settings')}><Settings size={18}/>Settings</button><button className="profile" onClick={()=>setDialog(API?'login':'connection')}><span className="avatar">{(user||'N')[0].toUpperCase()}</span><span>{user||(API?'Operator account':'Explore RepoPilot')}<small>{API?(user?'Signed in':'Sign in to RepoPilot'):'Read-only product preview'}</small></span><ChevronDown size={15}/></button></div>
  </aside>
  <main id="workspace-main">
   <header className="topbar"><div className="breadcrumb"><button id="desktop-nav-toggle" className="desktop-sidebar-toggle icon-button" aria-label={collapsed?'Expand sidebar':'Collapse sidebar'} title={collapsed?'Expand sidebar':'Collapse sidebar'} aria-expanded={!collapsed} aria-controls="workspace-navigation" onClick={toggleSidebar}><Menu size={20}/></button><button id="mobile-nav-toggle" className="mobile-menu icon-button" aria-label={mobile?'Close navigation':'Open navigation'} aria-expanded={mobile} aria-controls="workspace-navigation" onClick={()=>setMobile(!mobile)}><Menu size={20}/></button><span>Workspace</span><ChevronRight size={14}/><strong>{section}</strong></div><div className="top-actions"><a href="/guide/" className="icon-button" aria-label="How to use RepoPilot" title="How to use RepoPilot"><FileCode2 size={19}/></a><ThemeToggle/><span className="preview-badge">{API?'API configured':'Preview'}</span><a href="https://github.com/thanos07/repopilot" target="_blank" rel="noreferrer" aria-label="GitHub"><Github size={19}/></a></div></header>
   {!API&&<div className="preview-notice"><span><Code2 size={15}/> Read-only portfolio preview. Public signup and live execution are planned for a future update.</span><button onClick={()=>setSection('Settings')}>Preview details <ArrowUpRight size={14}/></button></div>}
   {API&&user&&!canOperate&&<div className="message" role="status">Live actions are restricted to the workspace operator.</div>}{error&&<div className="message error" role="alert">{error}<button aria-label="Dismiss error" onClick={()=>setError('')}><X size={16}/></button></div>}{notice&&<div className="message" role="status">{notice}<button aria-label="Dismiss notification" onClick={()=>setNotice('')}><X size={16}/></button></div>}
   ...
   {section==='Tasks'&&(!API||(!loading&&!!user&&!task.is_demo))&&<>
    ...
    <div className={'workspace-body '+(!rail?'wide':'')}><section className="task-content" role="tabpanel" aria-label={tab}>
     {tab==='Summary'&&<>
      ...
      {['DRAFT','FAILED'].includes(task.status)&&<button className="primary" disabled={busy||!canOperate} onClick={()=>action('/tasks/'+task.id+'/start/')}>{busy?<LoaderCircle className="spin" size={16}/>:<ArrowRight size={16}/>}{task.status==='FAILED'?'Retry saved task':'Start agent'}</button>}
      {isRunning&&<button className="secondary" disabled={busy||!canOperate} onClick={()=>action('/tasks/'+task.id+'/cancel/')}>Stop task</button>}
      {task.status==='AWAITING_APPROVAL'&&<div className="approval"><div><ShieldCheck size={22}/><div><strong>Ready for your review</strong><p>Inspect the diff and verification before publishing.</p></div></div><button className="primary" disabled={!task.is_demo&&!canOperate} onClick={()=>task.is_demo?setTab('Changes'):setDialog('approve')}>{task.is_demo?'Inspect example diff':'Review change'} <ArrowRight size={16}/></button></div>}
      {task.status==='APPROVED'&&<button className="primary" disabled={busy||!canOperate} onClick={()=>action('/tasks/'+task.id+'/publish/')}>Create draft pull request <GitPullRequest size={17}/></button>}
