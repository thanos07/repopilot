...
export default function Workspace(){
 ...
 return <div className={'app '+(collapsed?'sidebar-collapsed':'')}><a className="skip-link" href="#workspace-main">Skip to workspace</a>
  {mobile&&<button className="nav-scrim" aria-label="Close navigation" onClick={()=>setMobile(false)}/>}<aside id="workspace-navigation" aria-label="Workspace navigation" className={'sidebar '+(mobile?'open':'')}><button className="desktop-sidebar-close icon-button" aria-label="Collapse sidebar" title="Collapse sidebar" onClick={toggleSidebar}><PanelRightOpen size={20}/></button><button className="sidebar-dismiss icon-button" aria-label="Close navigation" onClick={()=>setMobile(false)}><X size={20}/></button>
   ...
   <button className="primary new-task" onClick={()=>setDialog(API?(user?'task':'login'):'connection')}><Plus size={17}/>New task<span>＋</span></button>
  ...
  {dialog&&<div className="modal-backdrop" onClick={e=>{if(e.target===e.currentTarget)setDialog('')}}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="dialog-title"><button className="modal-close icon-button" aria-label="Close dialog" onClick={()=>setDialog('')}><X size={20}/></button><h2 id="dialog-title">{dialog==='connection'?'Workspace access':dialog==='login'?'Sign in':dialog==='repository'?'Add a repository':dialog==='approve'?'Approve this exact patch?':'Create a task'}</h2>
   {dialog==='connection'?<><p>Preview mode lets you inspect an illustrative task. Running real tasks requires an available service and a RepoPilot account provided by the site administrator.</p><p>If you already have an account, sign in to continue. Otherwise, read the user guide for access requirements. Never enter provider keys or GitHub credentials in task forms.</p><button className="primary" onClick={()=>{setDialog('');setSection('Settings')}}>View settings <ArrowRight size={16}/></button></>:dialog==='approve'?<><p>Approval is bound to this patch and its base commit. Further edits require a new review.</p><p className="mono digest">{task.patch?.digest}</p><button className="secondary" onClick={()=>{setDialog('');setTab('Changes')}}>Inspect diff</button> <button className="primary" disabled={busy} onClick={async()=>{const d=await action('/tasks/'+task.id+'/approve/',{digest:task.patch?.digest});if(d)setDialog('')}}>Approve patch</button></>:<form onSubmit={submit}>
   {dialog==='login'?<><p className="muted">Use your administrator-provided RepoPilot account. Public registration is not available in this version. <a href="/guide/">Read the user guide</a>.</p><label>Username<input name="username" autoComplete="username" required autoFocus/></label><label>Password<input type="password" name="password" autoComplete="current-password" required/></label></>:dialog==='repository'?<label>Public GitHub repository URL<input name="url" type="url" placeholder="https://github.com/owner/repository" required autoFocus/></label>:<><label>Repository<select name="repository" required>{repos.map(r=><option key={r.id} value={r.id}>{r.full_name}</option>)}</select></label><label>Task title<input name="title" placeholder="Fix a specific bug" maxLength={240} required autoFocus/></label><label>Description<textarea name="description" rows={5} placeholder="Describe one small bug, the relevant file if known, and expected behavior." required maxLength={12000}/></label><p>New tasks use a $0.05 API estimate limit. Focus on one small Python fix.</p>{!repos.length&&<p>Add a repository before creating a task.</p>}</>}
