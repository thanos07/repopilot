...
class Command(BaseCommand):
    ...
    def add_arguments(self,p):
        p.add_argument('--username',required=True);p.add_argument('--limit',type=int,default=1);p.add_argument('--output',required=True)
    def handle(self,*args,**o):
        if not settings.LIVE_EXECUTION_ENABLED:raise CommandError('Enable live execution explicitly before paid evaluation.')
        if not 1<=o['limit']<=10:raise CommandError('limit must be 1 through 10.')
        ...
        rows=[]
        for path in sorted((settings.BASE_DIR.parent/'evals').glob('*/case.json'))[:o['limit']]:
